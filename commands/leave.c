...
long timezone;
#else
long timezone = 0 * HOUR;	/*  Should be in ctime(3)	  */
#endif
#endif
...
main( argc, argv )
  ...
  for(;;)
    ...
    /*  required because 16 bit ints don't allow long waits).	*/
...
Get_Hour_Min( when, hour, min )
  ...
  switch ( sscanf( when, "%d:%d", &hour_min, &just_min ) )
    {
    case 1 :  *hour = hour_min / 100;
	      *min  = hour_min % 100;
	      break;

    case 2 :  *hour = hour_min;
	      *min  = just_min;
	      break;
  ...
  if ( hour_min < 0  ||  just_min < 0  ||  *min > 59 )
	Usage();
  }
