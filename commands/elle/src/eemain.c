...
#ifdef STKMEM
	sbm_init(&stackm[0],STKMEM);	/* Initialize mem alloc rtns */
#endif /*STKMEM*/
#if SUN
	sun_main(&argc, argv);		/* On SUN, invoke window startup */
#endif /*SUN*/

	setbuf(stdout, NULL);		/* Remove all stdio buffering */
	setbuf(stderr, NULL);		/* in case of error reports. */
...
f_pshinf()
{
	...
	if (forkexec(&fx) < 0)
		writez(2,"Cannot run EXEC");
    }
#else /*-TOPS20*/
	switch(pid = fork())
	  {	case -1:
			writez(2,"Cannot fork");
			break;
