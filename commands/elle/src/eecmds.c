...
/* CMD_READ() - Read a command (single char) from user
 */
cmd_read()
{
...
tinhang()
{
	tbufls();		/* Ensure all output gone while we wait */
#if FX_SKMAC
        km_abort();
#endif /*FX_SKMAC*/
	unrchf = cmd_read();	/* Wait til something typed */
}

/* CMD_XCT(char) - Command Execution dispatch routine
 */
cmd_xct(ch)
{	register int (*funct) ();
	register int c;
...

/* CMD_FUN - Return function for char, 0 if none */
...
/* CMD_IDX - Given integer command char value, return function index for it */
...
{	char pfile[200];
	...
	def_prof.chrvec  += (SBMO) profptr;
	def_prof.metavec += (SBMO) profptr;
	def_prof.extvec  += (SBMO) profptr;
#if SUN
	def_prof.menuvec += (SBMO) profptr;
#endif /*SUN*/
	goto done;
