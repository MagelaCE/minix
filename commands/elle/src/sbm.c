...
{       register struct smblk *sm, *sml;
	...
	sbm_lmak(sm,sizeof(struct smblk),SMNODES);      /* Make list */
...
{       register struct smblk *sm, *smp;
	...
	sbm_lmak(smp, elsize, cnt);	/* Build freelist */
...
{       register struct smblk *sm, *sml;
	...
	if((sm = sbm_list) == 0         /* If never done, */
	  && (sm = sbm_init(0,0)) == 0)	/* initialize mem alloc stuff. */
		return(0);		/* Can't init??? */
