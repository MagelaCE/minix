...
{	register struct buffer *b;
	...
	sb_open(b,0);		/* Open buffer with null initial sbstring */
	b->b_fn = 0;
	b->b_flags = 0;
...
{	register struct window *w;
 ...
 redo:
	...
	"--END-- (type Space to restore display)",
			w->w_ht);
...
register struct window *bw;	/* Base window we are reporting status of */
{
	...
	if(mw->w_flags&W_STANDOUT)
		e_insn(SP, scr_wd0 - e_blen());	/* Stuff out with spaces */
#endif
