...
chroff sbx_qlen();
struct sdblk *sbx_ndget(), *sbx_ndel (), *sb_killn (), *sbx_beg (),
	     *sbx_xcis (), *sbx_scpy (), *sbx_next (),
	     *sbx_norm (), *sbx_sdcpy (), *sbx_split (), *sbm_nmak (),
		*sbx_ffnd();
SBMA sbx_malloc();
struct smblk *sbx_mget (), *sbm_mget (), *sbx_msplit (), *sbm_split ();
...
{	register struct sdblk *sd;
	...
	if((sd = sdp) == 0)
	  {	sd = sbx_ndget();	/* Get a fresh node */
		...
		return(0);		/* Perhaps could normalize tho */
...
{	register struct sdblk *sd;
	...
	if((fd = ifd) < 0 || SB_NFILES <= fd	/* Check for absurd FD */
	  || sbv_ftab[fd])			/* and slot already in use */
		return(0);
	if((len = sbx_fdlen(fd)) < 0) return(0);
...
register SBBUF *sb;
{
	...
	if(sb->sbcur == 0			/* Make sure sbbuffer there */
	  || sbx_ready(sb,SK_READF,0,SB_BUFSIZ) <= 0)  /* Normalize & gobble */
		return(EOF);
...
int ch;
{
	...
	if(sb->sbflags&SB_OVW)		/* If overwriting, handle std case */
	  {	if(sb->sbiop &&
		...
		if(sbx_ready(sb,SK_READF,0,SB_BUFSIZ) > 0) /* Re-normalize */
	...
	if(sbx_ready(sb,SK_WRITEF,SB_SLOP,SB_BUFSIZ) <= 0)	/* Get room */
...
register SBBUF *sb;
{
	...
	if(sbx_ready(sb,SK_READB,SB_BUFSIZ,0) <= 0)
		return(EOF);
	return(sb_rgetc(sb));
}
...
{	register SBBUF *sb;
	...
	else sb_open(sb,0);	/* No stuff left!  Create null sbstring */
...
{	register SBBUF *sb;
	...
	case SK_DELF:		/* Delete forward - force BOB */
		if(sb->sbrleft <= 0)		/* At end of blk? */
		  {	sbx_smdisc(sb);		/* Win, unhook */
			return((int)sbx_next(sb));   /* Return next or 0 if EOF */
		  }
		sbx_smdisc(sb);			/* Not at end, but see if */
		if(moff == 0)			/* at beg of blk? */
			return((int)sd);	/* Fast win! */
		break;
	...
	sekr3:	if(sb->sboff > cmin+slop)	/* Too much leading text? */
		...
		if(rem)		/* If disk not word-aligned, hack stuff */
		  {	sm = sbx_msplit(sm, rem);	/* Trim off from beg*/
   ...
   sekw:	sbx_sbrdy(sb);		/* Sets up sbwleft... */
	...
	case SK_DELF:		/* Delete forward */
		if(sb->sboff == 0)	/* At block beg already? */
			return((int)sd);	/* Win, return it */
		sbx_split(sd, sb->sboff);	/* No, split up and */
		return((int)sbx_next(sb));	/* return ptr to 2nd part */
	...
	case SK_DELB:		/* Delete backward (force EOB align) */
		...
		return((int)sd);	/* And return ptr to 1st part */
		break;
	...
	if(coff == 0)			/* If offset was 0, */
	  {				/* then 1st SD becomes null */
		...
		if(coff >= sd->sdmem->smuse)
			goto nulsdf;
		else sdf->sdmem = sbx_msplit(sd->sdmem, (int)coff);
...
Problems:
	...
	Could have count of # nodes free for SM and SD; don't GC unless
