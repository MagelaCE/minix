...
register FILE *infile;
{
  ...
  extern BYTE *diagname, *diagheader, buffer[BUFSIZE];
...
LONG *startpos, *endpos;
{
  ...
  if ((diagitem = diaglist->next) == NULL)	/* List is empty */
	...
	*endpos = diagitem->endpos;
	...
	free((BYTE *) diagitem);
	if (diaglist->next == NULL) diaglist->last = NULL;
  }
}
...
register BYTE *searchstring;
{
  ...
  while (!(stop || error))
	if (readbuffer(copybuffer, "archive", infile))
		...
		if (outfile != NULL) fputs(copybuffer, outfile);
...
register FILE *infile;
{
  ...
  extern BYTE *diagname, *diagheader, *repairname, *repairheader, buffer[BUFSIZE];
