...
primary()
{
	...
	case T_FUNCTION:
		...
		break;
	case T_VARIABLE:
/* cast for msc - right cast, really what is wanted? */
...
postincdec()
{
	...
	if ( Stackptr->lvalue )
	{
		if ( Stackptr->class )
		{
			 ...
			 *   cp++ = value;
			 */
/* right cast for msc, but is it right for bawk? */
...
/* casts for msc, are they right for bawk? */
