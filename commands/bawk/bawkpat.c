...
{
	/*
	 * Compile a regular expression from current input file into the
	 * given pattern buffer. 
	 */
	...
	char *patbuf;		/* destination pattern buffer */
{
	...
	*cp;			/* Pattern start     */
	...
	if ((c = getcharacter()) == -1)
		...
		/*
		 * Class exclusion, for example: [^abc] Swallow the "^" and
		 * set token type to class exclusion. 
		 */
		o = NCLASS;
	}
	else {
		/*
		 * Normal class, for example: [abc] push back the character
		 * and set token type to class 
		 */
		ungetcharacter(c);
		o = CLASS;
	}
